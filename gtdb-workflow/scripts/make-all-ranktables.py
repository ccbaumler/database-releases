...
def main():
    ...
    for n, ss in enumerate(db.signatures()):
        ...
        if n % 10 == 0:
            percent = (n + 1) / len(db) * 100
            print(f"...Processing {sig_name}: {n+1} of {len(db)} ({percent:.2f}%)", end="\r", flush=True)
        ...
        if os.path.exists(out_csv) or os.path.exists(out_csv_gz):
            print(f"Skipping {out_csv} or {out_csv_gz}: file already exists.")
            continue 
        ...
        per_sig_args = argparse.Namespace(**per_sig_kwargs)

        pangenome_ranktable_main(per_sig_args)
